...
var (
	...
	configPrefix   = []byte("smartshare-config-") // config prefix for the db
...
func WriteCanonicalHash(db sspdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db sspdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db sspdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db sspdb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db sspdb.Putter, count uint64) error {
...
func WriteHeader(db sspdb.Putter, header *types.Header) error {
...
func WriteBody(db sspdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db sspdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db sspdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db sspdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db sspdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db sspdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db sspdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db sspdb.Database) sspdb.Database {
	return sspdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db sspdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db sspdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db sspdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
