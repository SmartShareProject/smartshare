...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.smartshare", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for smartshare issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "smartshare-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/smartshareproject/smartsharetest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.smartshare", "github.com/smartshareproject/smartshare/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "smartshare.aar"), "smartshare.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "smartshare", "smartsharetest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.smartshare.smartsharetest"
...
dependencies {
    ...
    compile(name: "smartshare", ext: "aar")
}
`
