...
import (
	...
	"github.com/smartshareproject/smartshare/internal/jsre"
	"github.com/smartshareproject/smartshare/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jssp", struct{}{})

	jethObj, _ := c.jsre.Get("jssp")
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jssp);"); err != nil {
	...
	flatten := "var ssp = web3.ssp; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	if c.prompter != nil {
		...
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jssp.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jssp.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jssp.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jssp.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines ssp.getBalance(ssp.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	// Print some generic Smartshare metadata
	fmt.Fprintf(c.printer, "Welcome to the Smartshare JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + ssp.coinbase);
		console.log("at block: " + ssp.blockNumber + " (" + new Date(1000 * ssp.getBlock(ssp.blockNumber).timestamp) + ")");
