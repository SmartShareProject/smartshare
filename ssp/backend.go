...
import (
	...
	bftBackend "github.com/smartshareproject/smartshare/consensus/bft/backend"
	...
	"github.com/smartshareproject/smartshare/miner"
...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	log.Info("Initialising Smartshare protocol", "versions", ProtocolVersions, "network", config.NetworkId)

	if !config.SkipBcVersionCheck {
		bcVersion := core.GetBlockChainVersion(chainDb)
		if bcVersion != core.BlockChainVersion && bcVersion != 0 {
			return nil, fmt.Errorf("Blockchain DB version mismatch (%d / %d). Run smartshare upgradedb.\n", bcVersion, core.BlockChainVersion)
		}
...
func CreateDB(ctx *node.ServiceContext, config *Config, name string) (sspdb.Database, error) {
	db, err := ctx.OpenDatabase(name, config.DatabaseCache, config.DatabaseHandles)
	if err != nil {
		return nil, err
	}
	if db, ok := db.(*sspdb.LDBDatabase); ok {
...
func CreateConsensusEngine(ctx *node.ServiceContext, config *Config, chainConfig *params.ChainConfig, db sspdb.Database) consensus.Engine {
...
func (s *Ethereum) APIs() []rpc.API {
	apis := sspapi.GetAPIs(s.ApiBackend)
...
func (s *Ethereum) ChainDb() sspdb.Database            { return s.chainDb }
...
func (s *Ethereum) Start(srvr *p2p.Server) error {
	...
	s.netRPCService = sspapi.NewPublicNetAPI(srvr, s.NetVersion())
