...
import (
	...
	"github.com/smartshareproject/smartshare/tests"
)
...
var makeTest = function(tx, rewind) {
  ...
  var block   = ssp.getBlock(ssp.getTransaction(tx).blockHash);
  var genesis = ssp.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.ssp.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  ssp.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
