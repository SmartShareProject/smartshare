// Package sspstats implements the network stats reporting service.
package sspstats
...
import (
	...
	"github.com/smartshareproject/smartshare/common/mclock"
...
type Service struct {
	...
	eth    *ssp.Ethereum      // Full Ethereum service if monitoring a full node
...
func New(url string, ethServ *ssp.Ethereum, lesServ *les.LightEthereum) (*Service, error) {
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["ssp"]; info != nil {
		network = fmt.Sprintf("%d", info.(*ssp.NodeInfo).Network)
		protocol = fmt.Sprintf("eth/%d", ssp.ProtocolVersions[0])
	} else {
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the sspstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to sspstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to sspstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to sspstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	log.Trace("Sending pending transactions to sspstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	log.Trace("Sending node details to sspstats")
