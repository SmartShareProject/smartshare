...
import (
	...
	"github.com/smartshareproject/smartshare/signer/core"
	"github.com/smartshareproject/smartshare/signer/storage"
)
...
func (alwaysDenyUi) OnApprovedTx(tx sspapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUi) OnApprovedTx(tx sspapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(sspapi.SignTransactionResult{})
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := sspapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx sspapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
