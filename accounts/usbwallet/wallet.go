...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Smartshare
...
type wallet struct {
	...
	deriveChain    smartshare.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Smartshare account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain smartshare.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
// SignTx implements accounts.Walssp. It sends the transaction over to the Ledger
// wallet to request a confirmation from the user. It returns either the signed
// transaction or a failure if the user denied the transaction.
//
// Note, if the version of the Smartshare application running on the Ledger wallet is
