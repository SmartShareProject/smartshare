...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (sspdb.Database, error) {
	if ctx.config.DataDir == "" {
		return sspdb.NewMemDatabase()
	}
	db, err := sspdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
