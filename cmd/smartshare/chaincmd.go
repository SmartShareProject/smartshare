...
Use "smartshare dump 0" to dump the genesis block.`,
	}
)
...
func importPreimages(ctx *cli.Context) error {
	...
	stack := makeFullNode(ctx)
	diskdb := utils.MakeChainDatabase(ctx, stack).(*sspdb.LDBDatabase)
...
func copyDb(ctx *cli.Context) error {
	...
	db, err := sspdb.NewLDBDatabase(ctx.Args().First(), ctx.GlobalInt(utils.CacheFlag.Name), 256)
	if err != nil {
		return err
	}
	...
	start = time.Now()
	fmt.Println("Compacting entire database...")
	if err = chainDb.(*sspdb.LDBDatabase).LDB().CompactRange(util.Range{}); err != nil {
		utils.Fatalf("Compaction failed: %v", err)
	}
	fmt.Printf("Compaction done in %v.\n\n", time.Since(start))
